...
<?php
    ...
    if (!is_null($_POST['chara_id'])) {
      $word = implode(',', $_POST['chara_id']);
      
    } else {
      // $word = ""；
      echo '所持キャラクターにチェックを入れてください';
      exit();
    }
    ...
    LEFT JOIN
      special_course
    ON
      special_course.course_id = course.id
    WHERE
        -- special_course.chara_id = inplode()とsql IN()を組み合わせたものをここに書く。
        special_course.chara_id IN(' . implode(',', $_POST['chara_id']) . ')
    ) AS selected_course
