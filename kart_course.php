...
<?php
    ...
    if (!is_null($_POST['kart_id'])) {
      $word = implode(',', $_POST['kart_id']);
      
    } else {
      // $word = ""；
      echo '所持マシンにチェックを入れてください';
      exit();
    }
    ...
    (
      ...
      LEFT JOIN
        kart_course
      ON
        kart_course.course_id = course.id
      WHERE
        -- special_course.chara_id = inplode()とsql IN()を組み合わせたものをここに書く。
        kart_course.kart IN(' . implode(',', $_POST['kart_id']) . ')
    ) AS selected_course
